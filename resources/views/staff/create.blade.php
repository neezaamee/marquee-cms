...
<div class="card mb-3">
  <div class="card-header bg-light d-flex justify-content-between align-items-center">
    <h5 class="mb-0"><span class="fas fa-user-plus me-2 text-primary"></span>Add New Employee</h5>
  ...
  <div class="card-body">
    <form action="{{ route('staff.store') }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="row g-3">
        {{-- Profile Photo --}}
        <div class="col-12">
          <label class="form-label" for="photo">Profile Photo <small class="text-muted">(JPG/PNG, max 2MB)</small></label>
        ...
        <div class="col-md-6">
          ...
          <input class="form-control @error('name') is-invalid @enderror" id="name" name="name" type="text"
                 value="{{ old('name') }}" required placeholder="e.g. Muhammad Ali" />
        ...
        <div class="col-md-6">
          ...
          <input class="form-control @error('cnic') is-invalid @enderror" id="cnic" name="cnic" type="text"
                 value="{{ old('cnic') }}" required placeholder="e.g. 35202-1234567-1" maxlength="15" />
        ...
        <div class="col-md-6">
          ...
          <input class="form-control @error('mobile_number') is-invalid @enderror" id="mobile_number" name="mobile_number"
                 type="text" value="{{ old('mobile_number') }}" required />
      ...
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <input class="form-control @error('joining_date') is-invalid @enderror" id="joining_date" name="joining_date"
                 type="date" value="{{ old('joining_date', date('Y-m-d')) }}" required />
        ...
        <div class="col-md-6">
          ...
          <div class="input-group">
            ...
            <input class="form-control @error('salary') is-invalid @enderror" id="salary" name="salary"
                   type="number" step="0.01" min="0" value="{{ old('salary') }}" required placeholder="0.00" />
          </div>
        ...
        <div class="col-12">
          <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" id="enable_login" name="enable_login" value="1"
                   {{ old('enable_login') ? 'checked' : '' }} onchange="toggleLoginFields(this.checked)" />
            <label class="form-check-label fw-semi-bold" for="enable_login">
              Enable CMS Login Account for this employee
            </label>
            <small class="d-block text-muted">Only enable for staff who need access to the system (e.g. Managers, Booking Officers).</small>
          </div>
        </div>

        <div id="login-fields" class="{{ old('enable_login') ? '' : 'd-none' }}">
          <div class="row g-3">
            <div class="col-md-4">
              ...
              <input class="form-control @error('login_email') is-invalid @enderror" id="login_email" name="login_email"
                     type="email" value="{{ old('login_email') }}" placeholder="staff@marquee.com" />
            ...
            <div class="col-md-4">
              ...
              <div class="input-group">
                <input class="form-control @error('login_password') is-invalid @enderror" id="login_password"
                       name="login_password" type="password" placeholder="Min 6 characters" />
            ...
            <div class="col-md-4">
              ...
              <select class="form-select @error('login_role_id') is-invalid @enderror" id="login_role_id" name="login_role_id">
                ...
                @foreach($roles as $role)
                  <option value="{{ $role->id }}" {{ old('login_role_id') == $role->id ? 'selected' : '' }}>
      ...
      <div class="mt-4 d-flex justify-content-end gap-2">
        ...
        <button class="btn btn-primary btn-sm" type="submit">
          <span class="fas fa-save me-1"></span> Save Employee
        </button>
      </div>
    </form>
  </div>
...
<script>
  ...
  // Setup CNIC mask (format: XXXXX-XXXXXXX-X)
  function setupCnicMask(inputId) {
    const input = document.getElementById(inputId);
    if (!input) return;
    input.addEventListener('input', function() {
      let val = input.value.replace(/\D/g, '').substring(0, 13);
      if (val.length > 12) {
