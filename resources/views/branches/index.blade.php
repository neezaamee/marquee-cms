...
<div class="card mb-3">
  <div class="card-header bg-light d-flex justify-content-between align-items-center flex-wrap gap-2">
    ...
    <div class="d-flex align-items-center gap-2">
      <form action="{{ route('branches.index') }}" method="GET" class="d-inline-block">
        <div class="input-group input-group-sm">
          ...
          @if(request('search'))
            <a href="{{ route('branches.index') }}" class="btn btn-outline-secondary"><span class="fas fa-times"></span></a>
          @endif
        </div>
      </form>
              ...
              <td class="align-middle text-end px-3">
                <div class="d-flex justify-content-end gap-2">
                  ...
                  @if(auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('manage_settings'))
                    ...
                    <form id="delete-form-branch-{{ $branch->id }}" action="{{ route('branches.destroy', $branch->id) }}" method="POST" class="d-none">
                      @csrf
                      @method('DELETE')
                    </form>
                    <a class="btn btn-link p-0" href="javascript:void(0);" onclick="if(confirm('Are you sure you want to delete this branch?')) { document.getElementById('delete-form-branch-{{ $branch->id }}').submit(); }" data-bs-toggle="tooltip" title="Delete">
