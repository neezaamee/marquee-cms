<div>
    <div class="row g-3">
        <!-- Sidebar Form to Create/Edit Brand -->
        @if($showForm)
            <div class="col-md-4">
                <div class="card border border-200">
                    <div class="card-header bg-light">
                        <h5 class="mb-0">{{ $editId ? 'Edit Brand' : 'Create Brand' }}</h5>
                    </div>
                    <div class="card-body">
                        <form wire:submit.prevent="save">
                            <div class="mb-3">
                                <label class="form-label" for="brand-name">Brand Name *</label>
                                <input wire:model="name" class="form-control @error('name') is-invalid @enderror" id="brand-name" type="text" placeholder="e.g. National Foods" />
                                @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>

                            <div class="mb-3">
                                <label class="form-label" for="brand-desc">Description</label>
                                <textarea wire:model="description" class="form-control" id="brand-desc" rows="3" placeholder="Brand notes..."></textarea>
                            </div>

                            <div class="mb-3">
                                <label class="form-label" for="brand-status">Status *</label>
                                <select wire:model="status" class="form-select" id="brand-status">
        ...
        <!-- Main Brands Listing Table -->
        <div class="{{ $showForm ? 'col-md-8' : 'col-md-12' }}">
            <div class="card border border-200">
                <div class="card-header bg-light d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <h5 class="mb-0"><span class="fas fa-copyright me-2 text-primary"></span>Brands Management</h5>
                    <div class="d-flex align-items-center gap-2">
                        <!-- Search -->
                        <div class="input-group input-group-sm">
                            <input wire:model.live.debounce.300ms="search" class="form-control" type="search" placeholder="Search brands..." />
                        ...
                        @if(!$showForm && (auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('manage_inventory')))
                            <button wire:click="create" class="btn btn-falcon-primary btn-sm text-nowrap">
                                <span class="fas fa-plus me-1"></span>Add Brand
                            </button>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="table-responsive scrollbar">
                        <table class="table table-sm table-striped fs-10 mb-0 align-middle">
                            <thead class="bg-200 text-900">
                                <tr>
                                    <th class="px-3" style="width: 250px;">Brand Name</th>
                                    <th>Description</th>
                            ...
                            <tbody>
                                @forelse($brands as $brand)
                                    <tr>
                                        <td class="px-3 fw-semi-bold">{{ $brand->name }}</td>
                                        <td class="text-muted">{{ $brand->description ?: '—' }}</td>
                                        <td class="text-center">
                                            <span class="badge badge-subtle-{{ $brand->status === 'Active' ? 'success' : 'secondary' }} rounded-pill">
                                                {{ $brand->status }}
                                            </span>
                                        </td>
                                        <td class="text-end px-3">
                                            <div class="d-flex justify-content-end gap-2">
                                                @if(auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('manage_inventory'))
                                                    <button wire:click="edit({{ $brand->id }})" class="btn btn-link p-0" title="Edit">
                                                        <span class="text-primary fas fa-edit"></span>
                                                    </button>
                                                    <button class="btn btn-link p-0" type="button" data-bs-toggle="modal" data-bs-target="#deleteConfirmModal" wire:click="confirmDeletion({{ $brand->id }})" title="Delete">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body text-start">
                    <p class="mb-0 text-900">Are you sure you want to remove this brand? Items using this brand will remain but their brand reference will be cleared.</p>
                </div>
                <div class="modal-footer">
                    ...
                    <button wire:click="deleteRecord" type="button" class="btn btn-danger btn-sm" data-bs-dismiss="modal">
                        <span class="fas fa-trash-alt me-1"></span>Delete Brand
                    </button>
                </div>
            </div>
        </div>
